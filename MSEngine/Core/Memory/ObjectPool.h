...
private:
    ...
    std::array<size_t,PoolSize> m_freeIndices;
...
public:
    ...
    T* Allocate() {
        ...
        size_t index = m_freeIndices.at(m_top);
    ...
    void Free(T* ptr) {
        ...
        if (ptr < m_pool.data() || ptr >= (m_pool.data() + PoolSize)) {
        ...
        m_freeIndices.at(m_top) = index; // 그 자리에 방금 반납받은 빈 방 번호를 적어둠
    }
